...
const INITIAL_MESSAGES: Message[] = [
  {
    ...
    channel: "Coach",
    preview:
      "Nice work last session. For next time, increase squat depth slightly.",
    date: "2026-01-26",
    unread: true,
  },
  {
...
export default function UserDashboard() {
  function InvoiceRow({
    ...
    return (
      <div className="grid grid-cols-12 gap-4 px-5 py-4 items-center text-sm">
        ...
        {/* Payment */}
        <div className="col-span-2 font-semibold text-gray-900">{amount}</div>
  ...
  const [messages, setMessages] = useState<Message[]>(() => {
    ...
    if (!Number.isFinite(count) || count <= 0) {
      return INITIAL_MESSAGES.map((message) => ({ ...message, unread: false }));
    }
    ...
    const loadDashboard = async () => {
      ...
      try {
        ...
        const mappedSessions: Session[] = seances.map((s) => {
          ...
          const status: SessionStatus = statut.includes("term")
            ? "completed"
        ...
        const apiTx: DashboardTx[] = commandes.map((c: any) => {
          ...
          const createdAt = String(
            c?.date_commande || c?.created_at || new Date().toISOString(),
          );
          const statut = String(c?.statut || "").toLowerCase();
          const status: DashboardTx["status"] =
            statut === "annule"
              ? "failed"
              : statut === "attente"
                ? "pending"
        ...
        const localTx: DashboardTx[] = parsedInv
          .filter(
            (inv: any) =>
              !email ||
              String(inv?.customer_email || "").toLowerCase() ===
                email.toLowerCase(),
          )
          .map((inv: any) => ({
        ...
        const mergedTx = [...apiTx, ...localTx].sort((a, b) =>
          ...
          <div className="rounded-2xl bg-white border border-gray-100 p-8 shadow-sm">
            ...
            <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="rounded-2xl bg-[#FAFAF7] p-6 flex items-center justify-between">
                <div>
                  <p className="text-3xl font-extrabold text-green-700">
                    {loadingDashboard ? "..." : sessions.length}
                  </p>
              ...
              <div className="rounded-2xl bg-[#FAFAF7] p-6 flex items-center justify-between">
                <div>
                  <p className="text-3xl font-extrabold text-green-700">
                    {loadingDashboard
                      ? "..."
                      : new Set(sessions.map((s) => s.coachName)).size}
                  </p>
                  <p className="text-sm font-semibold text-gray-700 mt-1">
                    Total Coaches Booked
                  </p>
              ...
              <div className="rounded-2xl bg-[#FAFAF7] p-6 flex items-center justify-between">
                <div>
                  <p className="text-3xl font-extrabold text-green-700">
                    {loadingDashboard ? "..." : `${totalSpent.toFixed(2)} €`}
                  </p>
                  <p className="text-sm font-semibold text-gray-700 mt-1">
                    Payments
                  </p>
              ...
              <div className="flex items-center gap-4 md:col-span-1">
                ...
                <div>
                  <p className="text-sm font-semibold text-gray-900">
                    {todayAppointment?.title || "Aucun rendez-vous"}
                  </p>
                  <p className="text-sm text-gray-500">
                    {todayAppointment?.location || "-"}
                  </p>
                </div>
              </div>
